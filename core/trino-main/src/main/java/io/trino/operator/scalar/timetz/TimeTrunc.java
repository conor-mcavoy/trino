...
public final class TimeTrunc
{
            ...
            @SqlType("time(p) with time zone") long packedTime)
    {
        ...
        return packTimeWithTimeZone(truncate(picos, value, unit) / PICOSECONDS_PER_NANOSECOND, unpackOffsetMinutes(packedTime));
    }
    ...
    {
        return new LongTimeWithTimeZone(truncate(time.getPicoseconds(), value, unit), time.getOffsetMinutes());
    }

    private static long truncate(long picos, long value, Slice unit)
    {
        ...
        return picos / truncatePicoseconds * truncatePicoseconds;
    }
}
